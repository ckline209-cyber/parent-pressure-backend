...
- ⚠️→🛑 **Tried to build/run the Android app — blocked by hardware, not code.** Opened `parent-pressure-android` in Android Studio to sync Gradle. The dev machine turned out to have **only ~3.7GB of total RAM** (confirmed via `Get-CimInstance Win32_ComputerSystem`), well under Android Studio's stated 8GB minimum. First attempt crashed outright (native `mmap` OOM in the JVM). Tried tuning it down: added a custom `studio64.exe.vmoptions` (lower `-Xmx`, smaller code cache) and disabled the Gradle daemon / capped its heap in `gradle.properties`. That got further but hit new problems in sequence — `CICompilerCount` set too low (invalid, must be ≥2), code cache set too small (warning), then an overly aggressive `-XX:MaxMetaspaceSize=350m` cap that caused a repeating `OutOfMemoryError: Metaspace` loop on the UI thread (confirmed in `idea.log`), effectively breaking the IDE. Removed the metaspace cap entirely (both in `studio64.exe.vmoptions` and `gradle.properties`) as the last fix. **Decision: hold off on further Android Studio work on this machine — user is planning to upgrade to a machine with 8GB+ RAM rather than keep fighting this.**
...
## Hardware Transition Notes (read this first if working from a new machine)
The dev machine as of this session has only ~3.7GB RAM, which cannot comfortably run Android Studio. The plan is to resume Android work once upgraded to a machine with 8GB+ RAM. Here's what does and doesn't carry over automatically:

- **Both repos' committed code is safe regardless of the machine** — `parent-pressure-backend` and `parent-pressure-android` are fully committed and pushed to GitHub as of the end of this session. A new machine just needs `git clone` for each; nothing is stranded locally.
- **`.env` is NOT in git, by design** (see the git-history leak/cleanup above) — it only exists locally under `OneDrive\Documents\GitHub\parent-pressure-backend\.env`. OneDrive was confirmed running and syncing that folder on this machine, so it's very likely mirrored to OneDrive's cloud storage too, but this wasn't independently verified end-to-end. Before decommissioning this machine: confirm OneDrive shows "up to date," and/or save the current `DATABASE_URL` and `JWT_SECRET` values somewhere separate (password manager) as a cheap safety net — if lost, it's just a 2-minute Supabase password reset, not real data loss.
- **Not worth transferring, safe to leave behind:** `node_modules` in either repo (regenerate via `npm install`), the Android Studio/SDK installation itself, and the low-RAM tuning described above. A machine with 8GB+ RAM shouldn't need `studio64.exe.vmoptions` overrides at all — that file lives outside both repos (`%APPDATA%\Google\AndroidStudio<version>\`) and won't transfer with the code, which is fine since it's not needed on better hardware.
- **One thing that *did* get committed and will follow the repo:** the `gradle.properties` changes (`org.gradle.daemon=false`, `org.gradle.parallel=false`, `org.gradle.workers.max=1`, reduced `-Xmx`). These are harmless on a faster machine but are a low-RAM workaround, not a real requirement — worth reverting to Gradle's defaults (daemon on, parallel on) once on 8GB+ RAM, since disabling the daemon makes every build slower.

## Next Steps (in order)
1. **Once on a machine with 8GB+ RAM:** open `parent-pressure-android` in Android Studio, let it sync (generates the Gradle wrapper), and actually build/run it on an emulator with the backend running locally — try registering and logging in for real. Consider reverting the low-RAM `gradle.properties` settings first (see above).
...
- **Current dev machine has only ~3.7GB RAM — insufficient for Android Studio (8GB minimum stated by Google). Blocking Android app work until hardware is upgraded.**
