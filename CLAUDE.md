...
## Next Steps (in order)
1. **LLM personalization layer on top of the progression rule** (paid tier, per Business Model) — the deterministic rule now exists (`GET /workouts/exercises/:id/progression`); an LLM could add explanation/adjustment on top (e.g. accounting for reported soreness, plateaus, missed sessions) without doing the core arithmetic itself.
