...
- ⚠️ **Security incident: three secrets were exposed in the Claude Code chat transcript** while adding the Anthropic API key to `.env` — a shell command's redaction pattern missed a value with no `=` sign, and a follow-up file Read then displayed the whole file (`DATABASE_URL` with the Supabase DB password, `JWT_SECRET`, and `ANTHROPIC_API_KEY`) in plaintext in the session. **All three were flagged for rotation**; confirm before the next session whether that was actually done: (1) Supabase DB password reset, (2) `JWT_SECRET` regenerated (invalidates existing login sessions — fine, only test accounts exist), (3) old Anthropic key deleted and replaced at console.anthropic.com. `.env` itself was never committed to git (confirmed still gitignored) — this was a transcript exposure, not a repo leak.
- ✅ **LLM personalization layer on top of progressive overload** (the item that was top of Next Steps): `POST /workouts/exercises/:id/personalize` (paid tier, gated by `requirePremium`) takes optional `{soreness, notes}`, computes the same deterministic base suggestion, then calls Claude (`claude-haiku-4-5-20251001`, via `src/utils/aiCoach.js`) with that suggestion plus the last 5 logged sessions for context. The model can confirm or adjust the suggestion (e.g. hold weight steady if soreness is reported) but never does the core arithmetic itself — that stays in the deterministic rule. Reads `ANTHROPIC_API_KEY` from env and degrades gracefully (`ai_available:false`, base suggestion still returned) if it's missing, so nothing breaks without a key. Android's exercise log screen adds a None/Mild/Significant soreness picker and an "Ask AI Coach" button per exercise. **Verified live against the real Claude API**: reporting "mild" soreness correctly made the AI hold at the current weight instead of taking the rule's suggested +2.5kg; reporting no soreness had it agree with the rule and explain why using the actual logged history (e.g. referenced the 60kg→62.5kg trend, not just the last set).
- ✅ **Progressive overload suggestions** (the item that was top of Next Steps): `GET /workouts/exercises/:id/progression` is a **deterministic rule**, not an LLM call — it looks at the user's most recent `exercise_logs` row for that exercise and applies: hit target reps on every set at RPE≤8 → suggest +2.5kg (or +1 rep for bodyweight exercises with no logged weight); otherwise → suggest repeating the same weight/reps; no history yet → suggest the target reps with no weight. The exercise log screen fetches this per exercise on load and again after each new log, showing it as a highlighted hint above the log form. Verified live: an exercise with a prior successful set (62.5kg, all reps hit, RPE 7) correctly suggested 65kg; exercises with no history showed the "log your first set" message.
- ✅ **Exercise logging**: `GET /workouts/started/:id` (a started workout's exercises plus any logs so far), `POST /workouts/logs` (sets/reps-per-set/weight/RPE against an exercise, validating the requester owns the `user_workout` and the exercise exists), `PATCH /workouts/started/:id/complete`. Android's workouts screen now navigates to a new "Log Exercises" screen once a workout is started (tracked via a `workoutId -> userWorkoutId` map in `WorkoutsViewModel`), which lists each exercise with a reps/weight/RPE form and a "Complete Workout" button. Verified live end-to-end: started a workout, logged a real set on Barbell Back Squat, confirmed the row in `exercise_logs`, tapped Complete, confirmed `completed_date` set in `user_workouts`.
...
## Next Steps (in order)
1. **Confirm the three flagged secrets were actually rotated** (see the security incident note above) — Supabase DB password, `JWT_SECRET`, Anthropic API key. Do this before anything else if it hasn't happened yet.
...
6. The progression rule uses a flat +2.5kg increment for every weighted exercise regardless of lift type — a real program would use smaller increments for smaller/isolation lifts (e.g. bicep curls) than for big compound lifts (squat/deadlift). Worth revisiting once there's real usage data.
7. AI Coach responses aren't cached or rate-limited per user — a user could spam "Ask AI Coach" and rack up API costs. Worth a cheap guard (e.g. one call per exercise per workout session) once there's real usage.
